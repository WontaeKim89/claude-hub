...
#### 안전한 삭제 시스템 (Danger Zone)

모든 구성 요소(Skills, Plugins, Memory, CLAUDE.md, Agents, Commands, Hooks, MCP, Teams)에 삭제 기능을 통일된 패턴으로 제공한다.
...
| 대상 | 삭제 시 수행 | 확인 입력값 |
|------|-------------|------------|
| Custom Skill | `~/.claude/skills/{name}/` 디렉토리 삭제 | 스킬 이름 |
| Installed Skill | symlink 제거 (`~/.claude/skills/{name}`) | 스킬 이름 |
| Plugin | `claude plugin remove {name}` CLI 실행 | 플러그인 이름 |
| Agent | `~/.claude/agents/{name}.md` 삭제 | 에이전트 이름 |
| Command | `~/.claude/commands/{name}.md` 삭제 | 커맨드 이름 |
| Memory 파일 | `~/.claude/projects/{project}/memory/{file}` 삭제 | 파일 이름 |
| CLAUDE.md | 내용 초기화 (빈 파일로 덮어쓰기, 파일 자체는 유지) | "RESET" 고정 문자열 |
| Hook | settings.json에서 해당 hook 엔트리 제거 | 이벤트 타입명 |
| MCP Server | settings.json에서 해당 서버 제거 | 서버 이름 |
| Team | `~/.claude/teams/{name}/` 디렉토리 삭제 | 팀 이름 |

**공통 규칙:**
1. 삭제 전 반드시 BackupService로 자동 백업 생성
2. 확인 입력값이 정확히 일치할 때만 삭제 버튼 활성화
3. 삭제 후 토스트 메시지: "'{name}' 삭제됨. 백업에서 복원 가능합니다."
4. Dashboard의 Backup History에서 복원 가능

**프론트엔드 공통 컴포넌트:**

```
components/shared/DangerDeleteDialog.tsx

Props:
  - title: string          ("gen-pr 스킬을 삭제하시겠습니까?")
  - confirmText: string    ("gen-pr" — 사용자가 입력해야 하는 텍스트)
  - description?: string   (추가 경고 메시지)
  - onConfirm: () => void
  - onCancel: () => void
```

**구현 순서:**
1. `DangerDeleteDialog.tsx` 공통 컴포넌트 생성
2. 각 페이지의 기존 삭제 다이얼로그를 DangerDeleteDialog로 교체
3. CLAUDE.md 페이지에 "초기화" 기능 추가 (confirmText: "RESET")
4. Installed Skill 삭제 API 추가 (symlink 제거)
...
| Step | 내용 | 예상 파일 |
|------|------|-----------|
| 1 | DangerDeleteDialog 공통 컴포넌트 + 전 페이지 삭제 강화 | `components/shared/DangerDeleteDialog.tsx` + 각 페이지 수정 |
| 2 | Installed Skill 삭제 API (symlink 제거) + CLAUDE.md 초기화 API | `routers/skills.py`, `routers/claude_md.py` 수정 |
| 3 | SQLite 스키마 + DB 서비스 | `services/usage_db.py` |
| 4 | Hook tracker CLI (`claude-hub-tracker`) | `tracker.py` + pyproject.toml scripts |
| 5 | 세션 로그 파서 | `services/log_parser.py` |
| 6 | Stats API 라우터 | `routers/stats.py` |
| 7 | Dashboard Usage Stats UI | `pages/Dashboard.tsx` 확장 |
| 8 | Unused Items 경고 + Danger Zone 제거 버튼 | Skills/Plugins 페이지 통합 |
| 9 | pywebview 앱 창 모드 | `main.py` 확장 |
| 10 | `tracker install/uninstall/sync` CLI | `main.py` CLI 확장 |
